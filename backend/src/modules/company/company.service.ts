...
export class CompanyService {
  ...
  async getProfile(symbol: string) {
    const data = await this.http.withFallback(
      () => this.getProfileFromKbs(symbol),
      () => this.getProfileFromVci(symbol),
      'company.getProfile',
    );
  ...
  TickerPriceInfo(ticker: $ticker) {
    ...
    const result = await this.http.vciGraphql<any>(query, { ticker: symbol.toUpperCase() });
    return { message: MESSAGES.COMMON.SUCCESS, data: result?.data?.TickerPriceInfo || null };
  }

  async getShareholders(symbol: string) {
    const data = await this.http.withFallback(
      () => this.getShareholdersFromVci(symbol),
      () => this.getProfileFromKbs(symbol).then((p: any) => p?.Shareholders || []),
  ...
  OrganizationManagers(ticker: $ticker) { fullName positionName updateDate percentage quantity __typename }
    ...
    const result = await this.http.vciGraphql<any>(query, { ticker: symbol.toUpperCase() });
    return { message: MESSAGES.COMMON.SUCCESS, data: result?.data?.OrganizationManagers || [] };
  }
  ...
  Subsidiary(ticker: $ticker) { percentage subOrListingInfo { organName enOrganName } __typename }
    ...
    const result = await this.http.vciGraphql<any>(query, { ticker: symbol.toUpperCase() });
    return { message: MESSAGES.COMMON.SUCCESS, data: result?.data?.Subsidiary || [] };
  }

  async getEvents(symbol: string) {
    const data = await this.http.withFallback(
      () => this.getEventsFromKbs(symbol),
      () => this.getEventsFromVci(symbol),
      'company.getEvents',
    );
    return { message: MESSAGES.COMMON.SUCCESS, data };
  }
  ...
  async getNews(symbol: string, page = 1, limit = 10) {
    const data = await this.http.withFallback(
      () => this.getNewsFromKbs(symbol, page, limit),
      () => this.getNewsFromVci(symbol),
      'company.getNews',
    );
    return { message: MESSAGES.COMMON.SUCCESS, data };
  }
  ...
  async getForeignFlow(
    ...
    const res = await this.fetchIqPriceHistory(symbol, timeFrame, fromDate, toDate, page, size);
    ...
    const mapped = items.map((r: any) => ({
      ...
      matchNetVolume: (r.foreignBuyVolumeMatched ?? 0) - (r.foreignSellVolumeMatched ?? 0),
      matchNetValue: (r.foreignBuyValueMatched ?? 0) - (r.foreignSellValueMatched ?? 0),
      // GD Thỏa thuận
      ...
      dealNetVolume: (r.foreignBuyVolumeDeal ?? 0) - (r.foreignSellVolumeDeal ?? 0),
      dealNetValue: (r.foreignBuyValueDeal ?? 0) - (r.foreignSellValueDeal ?? 0),
      // Tổng
  ...
  async getProprietaryFlow(
    ...
    const mapped = items.map((r: any) => ({
      ...
      matchNetVolume: (r.totalMatchBuyTradeVolume ?? 0) - (r.totalMatchSellTradeVolume ?? 0),
      matchNetValue: (r.totalMatchBuyTradeValue ?? 0) - (r.totalMatchSellTradeValue ?? 0),
      // GD Thỏa thuận
      ...
      dealNetVolume: (r.totalDealBuyTradeVolume ?? 0) - (r.totalDealSellTradeVolume ?? 0),
      dealNetValue: (r.totalDealBuyTradeValue ?? 0) - (r.totalDealSellTradeValue ?? 0),
      // Tổng ròng
  ...
  async getSupplyDemand(
    ...
    const mapped = items.map((r: any) => ({
      ...
      unmatchedDiff: (r.totalBuyUnmatchedVolume ?? 0) - (r.totalSellUnmatchedVolume ?? 0),
      // Số lệnh đặt
      ...
      tradeVolumeDiff: (r.totalBuyTradeVolume ?? 0) - (r.totalSellTradeVolume ?? 0),
      // Volume + giá phiên
      totalVolume: r.totalVolume ?? 0,
  ...
  private async getInsiderFromVciIq(symbol: string, page: number, size: number) {
    const res = await this.http.vciIqGet<any>(
  ...
  private async getProfileFromKbs(symbol: string) {
    return this.http.kbsGet<any>(`/stockinfo/profile/${symbol.toUpperCase()}`, { l: 1 });
  }

  private async getEventsFromKbs(symbol: string) {
    return this.http.kbsGet<any>(`/stockinfo/event/${symbol.toUpperCase()}`, { l: 1, p: 1, s: 20 });
  }

  private async getNewsFromKbs(symbol: string, page: number, limit: number) {
    return this.http.kbsGet<any>(`/stockinfo/news/${symbol.toUpperCase()}`, { l: 1, p: page, s: limit });
  }
  ...
  News(ticker: $ticker, langCode: $langCode) { newsTitle newsImageUrl newsSourceLink publicDate newsShortContent __typename }
    ...
    const result = await this.http.vciGraphql<any>(query, { ticker: symbol.toUpperCase(), langCode: 'vi' });
